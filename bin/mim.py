...
# === New: Compare Articles with Each Other ===
from difflib import SequenceMatcher
...
article_texts = {}
for key, path in [("deseret", deseret_com_path), ("foxnews", foxnews_com_path), ("presidency", presidency_ucsb_edu_path)]:
    try:
        with open(path, "r", encoding="utf-8") as f:
            article_texts[key] = f.read()
    except:
        article_texts[key] = ""

print("\n=== Article to Article Similarity ===")
for k1 in article_texts:
    for k2 in article_texts:
        if k1 < k2:
            score = compare_texts(article_texts[k1], article_texts[k2])
            print(f"{k1} ↔ {k2}: {score:.3f}")

# === New: Compare Transcript to Articles ===
try:
    with open("data/Tim_Ballard_20250526_watermarked.minute.json", "r", encoding="utf-8") as f:
        transcript_data = json.load(f)
        transcript_text = " ".join([item["text"] for item in transcript_data.get("minutes", [])])
except:
    transcript_text = ""

# Mock translation (could be replaced by API or model)
translated_transcript = transcript_text  # Placeholder

print("\n=== Transcript to Article Similarity ===")
for name, content in article_texts.items():
    score = compare_texts(translated_transcript, content)
    print(f"Transcript ↔ {name}: {score:.3f}")
