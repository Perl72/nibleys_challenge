# transcribe_podcast.py — podcast to TTS pipeline 🎙️🧠🔊 + MEGADIAGNOSTICS
...
# === Manual logging until we init logger ===
print(">>> [BOOT] dt type before anything:", type(dt))
print(">>> [BOOT] dt repr:", dt)
print(">>> [BOOT] sys.modules['datetime']:", sys.modules.get('datetime'))
...
print(">>> [POST-utilities1] dt type:", type(dt))
print(">>> [POST-utilities1] sys.modules['datetime']:", sys.modules.get('datetime'))

import whisper
print(">>> [POST-whisper] dt type:", type(dt))

from TTS.api import TTS
print(">>> [POST-TTS] dt type:", type(dt))

# === Init logging ===
logger = initialize_logging()
logger.info(f"[🧠 DEBUG] dt type at logging init: {type(dt)}")
logger.info(f"[🧠 DEBUG] dt repr at logging init: {repr(dt)}")
logger.info(f"[🧠 DEBUG] sys.modules['datetime']: {sys.modules.get('datetime')}")
...
try:
    logger.info(f"[🧠 PRE-TIMESTAMP] dt type: {type(dt)}")
    logger.info(f"[🧠 PRE-TIMESTAMP] dt repr: {repr(dt)}")
    logger.info(f"[🧠 PRE-TIMESTAMP] sys.modules['datetime']: {sys.modules.get('datetime')}")
    ...
    logger.debug("[DEBUG] Exception traceback:", exc_info=True)
    sys.exit(1)
